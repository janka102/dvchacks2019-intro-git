...
function updateCharacterCount(messageLength) {
  var newColor = Math.round(
    255 * Math.min(1, messageLength / maxMessageLength)
  );
...
function sendMessage() {
  ...
  if (yakkerText.length > 0 && yakkerText.length <= maxMessageLength) {
    var time = new Date();
    var timeText =
    ...
    // clear out the textarea and reset the button and counter
    yakkerInputEl.value = '';
    updateButton(0);
